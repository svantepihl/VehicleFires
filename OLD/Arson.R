...
#anlagda bilbränder
dat_msb <- filter(dat_msb, dat_msb$Reason == "Arson")
...
dat_fires_stockholm_arson <- left_join(dat_stockholm[ ,-c(6)], Dates, by=c("Date", "Municipality_Code"))
...
write.csv(dat_days_stockholm_arson, "dat_days_stockholm_arson.csv")
write.csv(dat_fires_stockholm_arson, "dat_fires_stockholm_arson.csv")
